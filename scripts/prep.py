...
pa = Path(r"C:\Users\oscar10408\OneDrive\桌面\UMICH\2026 Winter\STATS 504\natality\data")

# Create a long form version of the birth data.
dl = []
for y in range(2011, 2021):
    bname = f"{y:04d}.txt.gz"
    f = pa / bname

    da = pd.read_csv(
        f,
        sep=",",
        dtype={"County Code": "string"},
        usecols=["County", "County Code", "Births"],
        engine="python",
        skipinitialspace=True,
    )

    da = da.dropna(subset=["County", "County Code", "Births"])
    da = da[da["County"].ne("Total")]
...
MAKE_2016_SUBSET = True

if MAKE_2016_SUBSET:
    f = pa / "us.1990_2023.20ages.adjusted.txt" 
    g = pa / "2016ages.txt.gz"          

    with open(f, "r", encoding="utf-8", errors="ignore") as inp:
        with gzip.open(g, "wt", encoding="utf-8") as out:
            for line in inp:
